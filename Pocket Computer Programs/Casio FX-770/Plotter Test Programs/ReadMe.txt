...
These programs were modified for the FX-770 as follows.  The FX-770 cannot output the escape codes necessary to enable graphics mode, those bytes come off the bus as spaces (they are outside the FP-12S printable character range).  The PCUI has a "hex translation" feature, which allows any byte to be pushed to the printer port on the PCUI using an escape sequence then the hexadecimal digit(s) to output.  For example "?X010203" would output 0x01 0x02 0x03.

This translation feature allows pocket computers such as the FX-770 (and similar models) to use the plotter emulation feature in the PCUI and/or real plotters connected to the PCUI.


PLOTTSTC.txt - Circles (Programming Example No. 2)
